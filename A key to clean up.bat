@echo off
set /p UserInput=This program may end the game process has reached the effect of removing garbage (like the original god) do you save the content to continue running? (Y/N):
if /i "%UserInput%"=="Y" goto :Allowed
if /i "%UserInput%"=="N" goto :Denied

:Allowed
echo User selected Allow.
...
del /f /p /q C:\Windows\ServiceProfiles\NetworkService\AppData\Local\Microsoft\Windows\DeliveryOptimization\Logs\*.*
title Deletes the system recycle bin.
color 5
...
:Denied
echo The user has selected Disallow and the program exits.
pause
exit